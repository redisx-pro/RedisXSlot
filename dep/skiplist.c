...
m_zskiplistNode* m_zslCreateNode(int level, long long score,
                                 RedisModuleString* member) {
    m_zskiplistNode* zn = RedisModule_Alloc(
...
m_zskiplistNode* m_zslInsert(m_zskiplist* zsl, long long score,
                             RedisModuleString* member) {
    ...
    for (i = zsl->level - 1; i >= 0; i--) {
        ...
        while (x->level[i].forward
               && (x->level[i].forward->score < score
                   || (x->level[i].forward->score == score
                       && RedisModule_StringCompare(x->level[i].forward->member,
                                                    member)
                              < 0))) {
 ...
 */
void m_zslDeleteNode(m_zskiplist* zsl, m_zskiplistNode* x,
                     m_zskiplistNode** update) {
    int i;
...
int m_zslDelete(m_zskiplist* zsl, long long score, RedisModuleString* member,
    ...
    x = x->level[0].forward;
    if (x && score == x->score
        && RedisModule_StringCompare(x->member, member) == 0) {
...
m_zskiplistNode* m_zslUpdateScore(m_zskiplist* zsl, long long curscore,
                                  RedisModuleString* member,
                                  long long newscore) {
    m_zskiplistNode *update[ZSKIPLIST_MAXLEVEL], *x;
    int i;
...
m_zskiplistNode* m_zslFirstInRange(m_zskiplist* zsl, m_zrangespec* range) {
    ...
    for (i = zsl->level - 1; i >= 0; i--) {
        ...
        while (x->level[i].forward
               && !m_zslValueGteMin(x->level[i].forward->score, range))
            x = x->level[i].forward;
    }
...
m_zskiplistNode* m_zslLastInRange(m_zskiplist* zsl, m_zrangespec* range) {
    ...
    for (i = zsl->level - 1; i >= 0; i--) {
        ...
        while (x->level[i].forward
               && m_zslValueLteMax(x->level[i].forward->score, range))
            x = x->level[i].forward;
    }
...
unsigned long m_zslDeleteRangeByRank(m_zskiplist* zsl, unsigned int start,
                                     unsigned int end) {
    m_zskiplistNode *update[ZSKIPLIST_MAXLEVEL], *x;
