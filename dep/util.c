...
int m_stringmatchlen(const char* pattern, int patternLen, const char* string,
        ...
        switch (pattern[0]) {
            case '*':
                ...
                if (patternLen == 1)
                    return 1; /* match */
                while (stringLen) {
                    if (m_stringmatchlen(pattern + 1, patternLen - 1, string,
                                         stringLen, nocase))
                        return 1; /* match */
                    string++;
                    stringLen--;
...
int m_stringmatch(const char* pattern, const char* string, int nocase) {
    return m_stringmatchlen(pattern, strlen(pattern), string, strlen(string),
                            nocase);
}
...
uint32_t m_sdigits10(int64_t v) {
    if (v < 0) {
        ...
        uint64_t uv
            = (v != LLONG_MIN) ? (uint64_t)-v : ((uint64_t)LLONG_MAX) + 1;
